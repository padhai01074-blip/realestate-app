...
class AppController extends Controller
{
    ...
    public function beforeFilter()
    {
        ...
        $frontmenuArr=array("Home"=>array("controller"=>"","action"=>"index","icon"=>"fa fa-home"),
                       ...
                       "Register"=>array("controller"=>"Registrations","action"=>"index","icon"=>"fa fa-user-plus"),
    ...
    private function _checkRoleAccess() {
        $user = $this->Session->read('frontUser');
        if ($user) {
            $this->loadModel('User');
            ...
            if ($userData) {
                ...
                $rolePermissions = array(
                    'buyer' => array('allowed' => array('pages', 'ourprojects', 'ourproperties', 'availabilities', 'contacts', 'users', 'registrations')),
                    'owner' => array('allowed' => array('pages', 'ourprojects', 'ourproperties', 'availabilities', 'contacts', 'users', 'registrations', 'listings', 'mydeals', 'mypayments')),
                    'agent' => array('allowed' => array('pages', 'ourprojects', 'ourproperties', 'availabilities', 'contacts', 'users', 'registrations', 'listings', 'mydeals', 'mypayments')),
                    'builder' => array('allowed' => array('pages', 'ourprojects', 'ourproperties', 'availabilities', 'contacts', 'users', 'registrations', 'listings', 'mydeals', 'mypayments')),
                    'admin' => array('allowed' => array('*')) // Admin has access to everything
                );

                if (isset($rolePermissions[$role])) {
                    $permissions = $rolePermissions[$role];
                    if ($permissions['allowed'] !== array('*')) {
                        $currentController = strtolower($this->params['controller']);
                        if (!in_array($currentController, $permissions['allowed'])) {
                            $this->Session->setFlash(__('Access denied. Insufficient permissions.'), 'error');
                            $this->redirect(array('controller' => 'users', 'action' => 'login'));
                        }
                    }
                }
